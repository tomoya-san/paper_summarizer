...
.PHONY: sync-env
sync-env:
	@grep NEXT_PUBLIC_COGNITO_USER_POOL_ID frontend/.env.local | cut -d= -f2 | tr -d '\n' | vercel env add NEXT_PUBLIC_COGNITO_USER_POOL_ID production --force
	@grep NEXT_PUBLIC_COGNITO_CLIENT_ID frontend/.env.local | cut -d= -f2 | tr -d '\n' | vercel env add NEXT_PUBLIC_COGNITO_CLIENT_ID production --force
	@grep NEXT_PUBLIC_API_URL frontend/.env.local | cut -d= -f2 | tr -d '\n' | vercel env add NEXT_PUBLIC_API_URL production --force